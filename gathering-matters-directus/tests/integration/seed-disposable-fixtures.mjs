...
// Submission IDs in production are generated by the database default uuidv7().
// The reviewer endpoint accepts canonical UUIDv7 only, so the fixtures must use
// real v7 identifiers (crypto.randomUUID() is v4 and would be rejected). This
// mirrors uuidv7() in JS so the seed is independent of the disposable DB's PG
// version.
function uuidv7() {
  const ts = Date.now();
...
for (const state of ['pending', 'clean', 'threats', 'unsupported', 'failed']) {
  review[state] = { submissionId: uuidv7(), fileId: crypto.randomUUID() };
